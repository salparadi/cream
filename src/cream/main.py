...
async def main(chain_name):
	...
	if (app_state.node == 'alchemy' and app_state.chain_name in ['base', 'optimism']) or (app_state.node == 'infura' and app_state.chain_name in ['avalanche']):
	...
	for s in (signal.SIGINT, signal.SIGTERM):
		loop.add_signal_handler(s, lambda: asyncio.create_task(shutdown(loop, all_tasks, api_service, app_state)))
...
def run():
	...
	parser = argparse.ArgumentParser(description="Run the block/transaction watcher for a specified chain.")
	parser.add_argument("chain", help="The chain name to load/use (ethereum/arbitrum/etc.)")
