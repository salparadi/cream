...
class BlockService:
	...
	async def get_alchemy_block_receipts(self, block_number):
		...
		initial_delay = 0.5 # Wait for X second(s) before the first try, adjust as needed
		delay = 1 # Wait for X second(s) before the first retry, adjust as needed
		
		await asyncio.sleep(initial_delay)  # Initial delay before starting the retries
		
		while retries < max_retries:
			async with self.app_state.http_session.post(self.websocket_uri, json=params) as response:
				response_data = await response.json()
				if 'error' not in response_data:
					receipts = response_data.get('result', {}).get('receipts', [])
					if receipts:
						# Filter out receipts of type 0x7e and process the rest
						filtered_receipts = [r for r in receipts if r.get('type') != '0x7e']
				...
				log.error(f"Error fetching block transactions: {data.get('error')}")
				
			retries += 1
			await asyncio.sleep(delay)
			delay *= 2  # Exponential backoff for the delay
		
		log.error(f"Failed to fetch block transactions for {block_number} after {max_retries} retries.")
	...
	async def get_infura_block_receipts(self, block_number):
		...
		while retries < max_retries:
			async with self.app_state.http_session.post(self.http_uri, headers=headers, json=params) as response:
				response_data = await response.json()
				#print(response_data)
				if 'error' not in response_data:
	...
	async def watch_new_blocks(self):
		...
		while True: 
			try: 
				async for websocket in websockets.client.connect(
					...
					while True:
						try:
							...
							if self.chain_name in ['base', 'optimism'] and self.node in ['alchemy']:
								await self.get_alchemy_block_receipts(block_number)
							
							if self.chain_name in ['avalanche'] and self.node in ['infura']:
								await self.get_infura_block_receipts(block_number)
							
						except asyncio.exceptions.CancelledError:
							return
						except websockets.exceptions.WebSocketException:
							log.exception("(watch_new_blocks) (websocket.recv) (WebSocketException)")
							break
						except Exception as e:  # Using generic exception temporarily, specify your exception
							log.exception(f"(watch_new_blocks) (websocket.recv) (catch-all): {e}")
							break
							
						start = time.perf_counter()
						
						self.app_state.newest_block = int(message["params"]["result"]["number"], 16)
						self.app_state.newest_block_timestamp = int(message["params"]["result"]["timestamp"], 16)
						
						block_times.append(self.app_state.newest_block_timestamp)
						self.app_state.average_blocktime = (block_times[-1] - block_times[0]) / (len(block_times) - 1)
						...
						fee_history_result = self.app_state.w3.eth.fee_history(1, "latest")["baseFeePerGas"]
						
						if len(fee_history_result) == 2:
							self.app_state.base_fee_last, self.app_state.base_fee_next = fee_history_result
						...
						log.info(f"[BLOCK #{self.app_state.newest_block}] "
				...
				log.exception("(watch_new_blocks) (websockets.ConnectionClosed) reconnecting...)")
				continue
