...
class TransactionService:
    def __init__(self, app_state: AppState):
        ...
        log.info(f"TransactionService initialized with app instance at {id(self.app_state)}")
    ...
    async def watch_transactions(self):
        ...
        while True:
            try:
                ...
                elif self.app_state.chain_name == "base" and self.app_state.node == 'node':
                ...
                elif self.app_state.chain_name == "optimism" and self.app_state.node == 'node':
                    await self.watch_node_pending_transactions()
                ...
                log.exception("(watch_pending_transactions) (websockets.ConnectionClosed) reconnecting...)")
                continue
    ...
    async def watch_alchemy_pending_transactions(self):
        async for websocket in websockets.client.connect(uri=self.websocket_uri, ping_timeout=None, max_queue=None):
            ...
            while True:
                try:
                    ...
                    log.exception("(watch_pending_transactions) (websockets.WebSocketException)")
                    break  # escape the loop to reconnect
                except asyncio.TimeoutError:
                    log.exception("(watch_pending_transactions) (asyncio.TimeoutError)")
                    break  # escape the loop to reconnect
                except Exception as exc:
                    log.exception(f"(watch_pending_transactions) (websocket.recv): {exc}")
    ...
    async def watch_arbitrum_sequencer_transactions(self, sequencer_uri: str):
        ...
        async for websocket in websockets.connect(uri=sequencer_uri, ping_timeout=None, max_queue=None):
            while True:
                try:
                    ...
                    try:
                        ...
                        for message in messages:
                            try:
                                l1_kind = message["message"]["message"]["header"]["kind"]
                                sender = message["message"]["message"]["header"]["sender"]
                                block_number = message["message"]["message"]["header"]["blockNumber"]
                            except Exception as e:
                                log.info(type(e))
                            ...
                            try:
                                ...
                                pending_transaction = decode_arbitrum_transaction(tx_bytes, tx_hash)
                            except Exception as exc:
                                log.error(exc)
                                continue
                            else:
