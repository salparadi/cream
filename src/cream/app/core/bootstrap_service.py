...
class BootstrapService:
    def __init__(self, app_state: AppState, chain_name: str):
        ...
        if not self.chain_info:
            log.error(f"Invalid chain name: {chain_name}. Please specify a valid chain from the supported list.")
        ...
        log.info(f"BootstrapService initialized with app instance at {id(self.app_state)}")
    ...
    async def start(self):
        try:
            ...
            log.info(f"Connected to {self.chain_name} (Chain ID: {chain_id}) at Block {newest_block}")
